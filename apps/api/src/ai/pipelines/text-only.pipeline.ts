...
export class TextOnlyPipeline {
  ...
  async run(message: string): Promise<AiChatResponse> {
    const textResults = await this.qdrant.searchTextByMessage(message, { limit: this.topK });
    const context = this.contextBuilder.fromTextResults(textResults);
    const answer = await this.gemini.generateTravelAnswer({ question: message, context });
    return this.formatter.format({ inputType: 'text_only', answer, textResults });
  }
}
