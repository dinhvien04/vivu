...
export default async function TimKiemPage({ params, searchParams }: PageProps) {
  const { locale } = await params;
      ...
      <>
        ...
        <main className="mx-auto max-w-container-max px-margin-mobile py-section-gap md:px-margin-desktop">
          <header className="mb-8 max-w-3xl">
            ...
            <h1 className="mt-2 font-h1 text-h1 text-on-surface">{t('search.headline')}</h1>
          ...
          <div className="mt-12 grid grid-cols-1 gap-4 sm:grid-cols-3">
            ...
            >
              ...
              <h3 className="mt-3 font-h4 text-h4 text-on-surface">
                {t('search.cardExploreTitle')}
              </h3>
              <p className="mt-1 text-body-sm text-on-surface-variant">
                {t('search.cardExploreBody')}
              </p>
            </Link>
            <Link
            ...
            >
              ...
              <h3 className="mt-3 font-h4 text-h4 text-on-surface">
                {t('search.cardCultureTitle')}
              </h3>
              <p className="mt-1 text-body-sm text-on-surface-variant">
                {t('search.cardCultureBody')}
              </p>
            </Link>
          </div>
        </main>
        ...
        <header className="mb-6">
          <nav aria-label="Breadcrumb" className="mb-4 text-label-md text-on-surface-variant">
            <ol className="flex flex-wrap items-center gap-2">
              ...
              <li aria-hidden>/</li>
              <li className="text-primary">{t('search.title')}</li>
            </ol>
          </nav>
          ...
          {q ? (
            <p className="mt-4 font-sans text-body-lg text-on-surface-variant">
              {total > 0
                ...
                : t.rich('search.noResultsForQuery', {
                    q,
                    strong: (chunks) => (
                      <span className="font-semibold text-on-surface">{chunks}</span>
                    ),
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
          ...
          <aside className="lg:col-span-3">
            <div className="sticky top-24 space-y-6 rounded-2xl border border-outline-variant bg-surface px-5 py-5">
              <div className="flex items-center justify-between">
                <h2 className="font-h4 text-h4 text-on-surface">{t('common.filter')}</h2>
                {activeFilterCount > 0 && (
                  <Link
          ...
          <section className="lg:col-span-9">
              ...
              <div className="inline-flex rounded-lg border border-outline-variant bg-surface-container p-1">
                {(
                  [
                    ...
                    { v: 'list' as const, icon: 'view_list', key: 'viewList' as const },
                  ...
                  >
                    ...
                    <span className="hidden sm:inline">{t(`search.${opt.key}`)}</span>
                  </Link>
                ))}
              </div>
              <div className="flex items-center gap-2">
                <span className="text-label-md text-on-surface-variant">{t('common.sort')}:</span>
                {(
                  [
                    { slug: 'recent' as const, key: 'recent' as const },
                    { slug: 'name' as const, key: 'name' as const },
                  ] as const
                ).map((s) => (
                  <Link
                    key={s.slug}
            ...
            {places.length === 0 ? (
              <EmptyState
                icon="search_off"
                title={q ? t('search.noResultsTitle', { q }) : t('search.noResults')}
                description={t('search.noResultsHint')}
                action={{ label: t('search.viewAllPlaces'), href: '/kham-pha' }}
              />
            ) : view === 'list' ? (
              ...
              <div className="space-y-3">
                <PlacesMapLoader places={places} height="65vh" locale={locale} />
                <p className="text-body-sm text-on-surface-variant">
                  {t('search.mapHint', {
                    geo: places.filter((p) => p.geo).length,
                    total: places.length,
                  })}
                </p>
              </div>
