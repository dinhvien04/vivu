...
```
- **Behavior**: Calls to `/api/v1/ai/chat` will immediately throw a `503 Service Unavailable` with the message: *"Vivu AI đang tạm bảo trì, vui lòng thử lại sau."*
...
```
- **Behavior**: Calls to `/api/v1/trip-plans/generate` will return `503 Service Unavailable` with: *"Tính năng tạo lịch trình AI đang tạm bảo trì, vui lòng gửi yêu cầu tư vấn để Vivu hỗ trợ."* The UI will display a link guiding users to `/tu-van`.
