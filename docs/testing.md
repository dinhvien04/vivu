...
- **ESLint**: Quét toàn bộ mã nguồn để phát hiện code thừa, sai cấu trúc hoặc không tuân thủ quy ước coding style của dự án.
  - _Lệnh thực thi_: `pnpm lint`
- **TypeScript Check**: Trình biên dịch tsc kiểm tra tính hợp lệ của kiểu dữ liệu (Types).
  - _Lệnh thực thi_: `pnpm typecheck`
  - _Lưu ý_: Script này tự động chạy bước sinh mã Prisma Client (`prisma:generate`) trước để tránh lỗi thiếu kiểu dữ liệu từ database.
...
- **Ví dụ Mocking trong AI Chat Service Test**:
  ...
  ```typescript
  jest
    .spyOn(aiTextGenerationService, 'generateTravelAnswer')
    .mockResolvedValue('Đây là câu trả lời giả lập về Biển Hồ');
  ```
...
- **Nguyên tắc viết E2E test**:
  ...
  - _Ví dụ_: `page.locator('[data-testid="btn-submit-lead"]').click()`.
