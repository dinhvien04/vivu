...
Môi trường production của Vivu được vận hành trên hạ tầng Serverless của **Vercel** để đảm bảo tốc độ tải trang nhanh và tự động co giãn lưu lượng.
...
*   Neon DB hỗ trợ sao lưu tự động dạng snapshot hằng ngày (giữ trong vòng 30 ngày).
...
### C. Khôi phục chỉ mục Vector Qdrant
Dữ liệu vector trên Qdrant Cloud có thể được khôi phục hoàn toàn từ cơ sở dữ liệu gốc:
1.  Đảm bảo PostgreSQL đã nạp đầy đủ thông tin địa danh.
2.  Chạy script re-index để backend tự động đọc dữ liệu text, gọi Qdrant Inference sinh vector và lưu lại vào collection:
    ```bash
