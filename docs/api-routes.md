...
- **Request Body**:
  ...
  - _Lưu ý_: Request bắt buộc phải có ít nhất `message` hoặc `image`.
- **Response (200 OK)**:
  ```json
  {
    "success": true,
...
- **Request Body**:
  ...
  ```
  - _Lưu ý_: Trường `website` là honeypot field dùng để chống spam. Nếu trường này có giá trị (do bot tự điền), API sẽ lập tức từ chối xử lý và trả về mã thành công giả hoặc lỗi.
